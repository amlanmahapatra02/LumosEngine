...
Skybox::Skybox(std::vector<std::string> faceLocation)
{
	...
	{
		unsigned char* textData = stbi_load(faceLocation[i].c_str(), &width, &height, &bitDepth, 0);
		if (!textData)
		{
			std::cout << "Failed to find : " << faceLocation[i].c_str() << std::endl;
			return;
		}
...
{
	// All  the transform values are stored in the 4th coloum of an 4x4 matrix
